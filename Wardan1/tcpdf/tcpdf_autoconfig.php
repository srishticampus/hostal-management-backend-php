...
$query = "SELECT user.name,user.email,user.contact_number,outpass.pass_date,outpass.pass_time,outpass.purpose,outpass.return_time,outpass.id,outpass.outpass_status,wardan.hostal_name FROM outpass INNER JOIN user ON outpass.user_id = user.id inner join wardan on wardan.id=user.hostal_id where outpass.id=$id";
 //echo $query;die();
